...
home-manager from **two** options (both declared in `modules/options.nix`,
generic class), rendered in this order:

1. `my.pi.agentInvariants` — one `# Invariants` block at the top. Prohibitions
   only, and only ones no guardrail can enforce for us. This is the block that
   also gets injected into delegated subagent prompts, which never see the rest
   of the file, so admission is strict: a **procedure** ("how to do X well")
   belongs in a policy section, a capability bundle, or a skill, where it loads
   only when X is happening; and anything a permission gate or a withheld tool
   can enforce should be enforced there and deleted from prose. String values
   only — a sops path decrypted at activation time is not available to a child.
2. `my.pi.globalAgentPolicies` — the policy sections. For the interactive
   session; may be long and may exercise judgment.

Both merge additively across modules, so the corporate flake can extend either.
See `docs/pi-subagents-rollout.md` for why the split exists.

In each attrset, every key becomes a named section; keys are sorted
alphabetically before concatenation, so use numeric prefixes to control order:
